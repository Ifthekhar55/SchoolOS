...
export const TeacherImport: React.FC<TeacherImportProps> = ({
  schoolId,
  onClose,
  onSuccess,
}) => {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const teachersWithSchool = data.map(teacher => ({
        ...teacher,
        schoolId,
      ...
      const result = await teacherApi.importTeachers(teachersWithSchool);
      setResult(result);
      ...
      setError(error.message || 'Failed to import teachers');
    } finally {
      setLoading(false);
    }
  };
  ...
  const downloadTemplate = () => {
    const headers = [
      'name', 'nameBangla', 'email', 'phone', 'employeeId', 'designation',
      'department', 'qualification', 'experience', 'specialization',
      'joiningDate', 'birthDate', 'gender', 'bloodGroup', 'religion',
      'nationality', 'address', 'addressBangla', 'emergencyContact'
    ];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
      <div className="w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            <h2 className="text-lg font-bold text-slate-900">Import Teachers</h2>
